...
def prune_network(args, network=None):
    device = torch.device("cuda" if torch.cuda.is_available() and args.gpu_no >= 0 else "cpu")
...
def get_channel_index(kernel, num_elimination, residue=None):
    # Calculate L1 norm of each channel in the kernel
    l1_norms = torch.sum(torch.abs(kernel.view(kernel.size(0), -1)), dim=1)
    if residue is not None:
        # If independent strategy is used, include its effect
        l1_norms += torch.sum(torch.abs(residue.view(residue.size(0), -1)), dim=1)

    # Sort indices by L1 norm in ascending order (we want the smallest for removal)
    _, args = torch.sort(l1_norms)
    # Select indices corresponding to the smallest norms
